...
from Motif_dist import data_dir, pval, file_place, cutoff_qval, perm, mut, enhancers


# Modify these values as needed
scatter_dir = rf"C:\Users\izzye\OneDrive - Johns Hopkins\Documents\BRIGHT\motif{mut}\full\images\scatterplots"  # Directory to save scatterplots
...
for enhancer in enhancers:
    bedgraph_files.extend([perm + enhancer + "_windows.bedgraph", perm + enhancer + "_randoms.bedgraph"])
...
if __name__ == "__main__":
    ...
    # Run pvalue for all window/random pairs automatically
    for window_file in bedgraph_files:
        if "_windows.bedgraph" in window_file:
            
            # Find corresponding random file
            random_file = window_file.replace("_windows.bedgraph", "_randoms.bedgraph")
            label = window_file.replace(f"{perm}", "").replace("_windows.bedgraph", "")
            if random_file in bedgraph_files:
                pvalue(window_file, random_file, label)
            else:
                print(f"Warning: No matching random file for {window_file}")
